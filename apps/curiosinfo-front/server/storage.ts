...
class FileStorage implements IStorage {
  ...
  async getAllTopics(): Promise<Topic[]> {
    ...
    const mapped = raw.map((it: any) => {
      return {
        ...
        startAt: it.startAt ?? it.start_at ?? null,
        endAt: it.endAt ?? it.end_at ?? null,
        created_at: it.created_at ?? it.created_at ?? null,
        updated_at: it.updated_at ?? it.updated_at ?? null,
      } as any;
    });
    return this.parseDateFields(mapped, ["created_at", "updated_at", "startAt", "endAt"]);
  }
  ...
  async getTopicBySlug(slug: string): Promise<TopicWithDetails | undefined> {
    ...
    const [normalizedTopic] = this.parseDateFields([t], ["created_at", "updated_at", "startAt", "endAt"] as string[]);
  ...
  class DatabaseStorage implements IStorage {
    ...
    async getTopicBySlug(slug: string): Promise<TopicWithDetails | undefined> {
      ...
      const normalizedTopic: any = {
        ...
        startAt: (topic as any).startAt ?? (topic as any).start_at ?? null,
        endAt: (topic as any).endAt ?? (topic as any).end_at ?? null,
